...
As far as we can tell, overlays do not support stacking: after unmapping an overlay that overlaid an existing memory mapping, that original memory mapping is not automatically re-mapped.  %vmspace_lwc_merge
Note that in contrast to memory and file descriptors which can be mapped by ranges, credentials can only be mapped whole or nothing\todo{proof is in code, language}. 

Overlaying resources from an lwC descriptor \lstinline{src} further requires that this descriptor is equipped with \textit{access capabilities} for each requested resource.
The rules of this access capability system are as follows:
\begin{itemize}
  ...
  \item After lwC creation, the child receives the lwC descriptor of its creator in \lstinline{new}, equipped with access capabilities to ranges flagged appropriately on lwcCreate\todo{mention \lstinline{LWC_MAY_ACCESS}?}.
  \item Access capabilities cannot be extened.
  \item Access capabilities can be reduced with the \lstinline{lwcRestrict(lwc, rspecs)} system call: the resource ranges specified in \lstinline{rspecs} can no longer be \lstinline{lwcOverlay}d throuh the \lstinline{lwc} descriptor.
  \item It is possible to pass lwC descriptors between lwCs --- they are not special with regards to file descriptor overlays.
\end{itemize}
In order to grasp the access capability system, we find it helpful to look at the implementation:
the overlay permission check and \lstinline{lwcRestrict} encode the current access capabilities of a file descriptor as the kernel representation of \lstinline{rspecs} in lwC file descriptor table entry (struct filedescent).

Note that revocation of existing overlays is not possible, i.e., \lstinline{lwcRestrict} only affects future mappings.\todo{proof, not mentioned in paper / impl. see microkernel discussions}
...
When compartmentalizing an application into different lwCs, it can be desirable to limit the system calls an lwC may perform, e.g., to limit filesystem access to a subdirectory or prohibit any network communication.
...
An alternative design could have the trap-handling lwC invoke the allowed syscalls itself and return the result as the opaque argument in an \lstinline{lwcSwitch} to the trapping lwC.
...
\lstinline{lwcSyscall} avoids the overhead of the overlay syscalls that would be required in that alternative design:
...
\lstinline{lwcSyscall} is integrated with the access capability system explained in the previous section:
