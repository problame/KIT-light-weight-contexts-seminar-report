...
These effects have been well-known for decades, e.g., in micro-kernel research\todo{cite one of the mkc-papers / aos papers on tlb pressure}  and implementation.
...
\item if nginx: switch out of lwC if I/O blocks, goto \ref{crit:enum:tx},
\item if apache: block (context switch to other thread in other lwC) until wakeup, then goto \ref{crit:enum:tx},
...
Thus, \textbf{28 or 115 responses} fit into the standard initial 32KiB tx buffer of the FreeBSD TCP stack~\cite{fbsd11tcpsendspace}
Assuming non-blocking reads for all request payloads\footnote{Requests are very short and likely pipelined.}, this means that session lengths of up to 28 or 115 requests per session are handled without any intermediate context switching.
...
Fourth, the evaluation setup might have been network-bound, which  might have needlessly disrupted or distorted the measurements:
The scalability test achieves a peak throughput of 180k req/sec for 40B and $\approx95k$ req/sec for 900B, which amounts to at least
% [GET/S] * [ETH_FRAME_SIZE]
% (180000 * 350 byte ) in giga bit
% (95000 * 1212byte ) in giga bit
0.5 or 0.92 or Gbit/s response Ethernet traffic according to our experiments (ree~\ref{attach:httppayloads}).
We recommend repeating the experiment with a 10 Gbit/s link to exclude\todo{ausschließen} any flow / congestion control issues.
...
We believe that the \textit{mechanism} for TLB tag exhaustion must be addressed by the virtual memory (VM) system.
But the lwC design likely breaks implicit assumptions of the VM system designers with regards to frequence of AS creation and switching.
Thus, lwCs must provide an appropriate multiplexing and scheduling \textit{policy} and measure its effects in application-level benchmarks, also addressing fairness in multi-tenant systems (multi-user, jails, containers).
