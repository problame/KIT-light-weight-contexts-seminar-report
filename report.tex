...
Application developers can leverage lwCs to achieve application protection goals in a more fine-grained and light-weight manner compared to traditional process-based privilege separation.\todo{quote provos / link related work}
...
in an uncompartmentalized \appname instance, the attacker could read the entire process's memory to extract the private key of the TLS key pair or access other user's images being handled in concurrently executing requests.
In contrast, the lwC-enabled \appname uses separate lwCs for the TLS handshake to protect the private key, as well as separate lwCs for each request to protect per-request image data.
