...
The run-time trusted computing base of a process that uses light-weight contexts (\textit{lwC-process}) is the
hardware,
firmware,
...
Specifically, an attacker may access any mapped memory through unprivilieged instructions and invoke any system call, including lwC management calls.

\textbf{Protection Goals}\hspace{1em}
Light-weight contexts enable the creation of multiple protection domains whithin a single process.
Therefore, we demand that the lwC abstraction must not weaken the protection between different processes.
Within a process using lwC's, it is the programmer's responsibility to determine the application-specific protection goals and to satisfy them using the security guarantees provided by lwCs.

\textbf{lwC Requirements}\hspace{1em}
An lwC consists of resource mappings for address space, file descriptor table and credential.
The kernel implementation of \texttt{lwcSwitch} guarantees that syscall handlers and memory management unit will perform their respective access permission checks against the resource mappings of the lwC that is being switched in.
Under that assumption, the security guarantees provided by lwCs rely solely on the soundness of the manipulation of those resource mappings through the lwC API, i.e., the modifications possible through \lstinline{lwcCreate}, \lstinline{lwcRestrict} and \lstinline{lwcOverlay}.

The only remaining lwC API is \lstinline{lwcSyscall}:
...
Note that this is different from \lstinline{lwcSwitch}, which only allows switching \textit{all three resource mappings} atomically before executing code in the context of the target lwC.
We therefore demand that it must not be possible to extend the set of \textit{passing} permission checks in syscall handlers or memory management unit by invoking \lstinline{lwcSyscall} with a \lstinline{mask} that combines resource mappings from trapping and trap-handling lwC, compared to invoking the syscall directly from either the trapping or the trap-handling lwC.

\textbf{Application Compartmentalization}
Under the assumption that the kernel lwC implementation meets above requirements, application developers can leverage lwCs to meet application protection goals.
However, it is to developers to verify that 
We restrict ourselves to a discussion of the example use case of \textit{application compartmentalization}

%y example of a network server that processes data and uses encryption:
%
%individual components of an application, such as the code and data required to encrypt a 
%
%Under the assumption of an lwC implementation that satisfies above requirements, application developers must bear the burden\todo{language?} of lwCs to %increase the confidentiality, integrity and availability guarantees     must use  the security properties that an lwC- much of the security guarantees of an %lwC-enabled application onto the application developer:
%\textbf{confidentiality \& integrity} of code and data accessible by an lwC depend on the resource-sharing specified on creation, as well as the use of %dynamic resource sharing.
%\textbf{availability} is not 
%sharin  the access capabilities overlay rights 

%Hardware-based attacks are excluded or assumed to be mitigated by the OS, including side-channels and exploitation of out-of-order \& speculative %execution.\todo{correct to exclude ooo and spec}
