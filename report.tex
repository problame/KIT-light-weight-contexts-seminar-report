...
\begin{itemize}
  ...
  \item its relatively small implementation footprint (code size, locality),
...
\end{itemize}
However, we believe that the evaluation requires significant refinements for it to be a convincing argument for the practicality of the design.
...
These effects have been well-known for decades, e.g., in micro-kernel research\todo{cite one of the mkc-papers / aos papers on tlb pressure}  and implementation, and should be measured in the application benchmarks using performance counters.

Second, the evaluation is performed on a machine with tagged TLB --- untagged TLB systems are not discussed.

% nice that is in fact applicable
% no audit that protection is actually achieved
reached
% but:
