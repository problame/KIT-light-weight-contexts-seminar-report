...
Multi-threaded forking is a well-known problem, \todo{citation} and the authors recommend applications to use barrier-synchronization with the creator.\todo{cite}
...
Shared memory, either through resource-specifiers on \lstinline{lwcCreate} or \lstinline{lwcOverlay} or through \lstinline{mmap(..., MAP_SHARED)} stays shared across forks.
...
Memory safe languages systematically eliminate the need for memory compartmentalization and unintended remote code execution.
...
Light-weight context appear to be less invasive with regards to program modification and do not treat memory differently from\todo{preposition} other resources.
...
\end{enumerate}
Our own experiments\ref{appendix:httppayloads} show 1146B response payloads for 900B requests and 284B response payloads for 40B requests.
...
To exclude\todo{ausschliessen} flow or congestion control issues, we recommend repeating the experiment with a 10 Gbit/s link.
...
In this report, we have presented the design of light-weight contexts and the evaluation of the author's implementation in FreeBSD 11.0.
...
Either way, we agree with the well-founded arguments against any further proliferation of fork semantics due to its composability problems, thread-unsafety and security implications (implicit instead of explicit sharing).\cite{baumann2019a}
...
However, we have found the the performance benchmarks for the modified applications to be incomplete and unrepresentative of real-world work loads.
