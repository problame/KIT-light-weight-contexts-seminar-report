...
followed by a brief security analysis in section~\ref{security}, which also includes an example use case for\todo{of?} lwCs in application security hardening.
We briefly discuss the author's implementation of lwCs in FreeBSD 11.0 in section~\ref{impl} and then proceed with a discussion of the evaluation results of the original paper (section~\ref{eval}).
...
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,mathescape,label=design:fig:switchpseudocode,caption=Pseudo code for lwcSwitch.,frame=trbl]
  syscall
...
By default, a new lwC is a snapshot copy of the current lwC's \textbf{resources} \textit{address space}, \textit{file descriptor table} and \textit{credential}.
...
\lstinline{lwcOverlay then checks that the requested overlay \lstinline{rspec} is a subset of the permitted-overlay mask.
...
It can be desirable to limit the system calls an lwC may perform, e.g., to limit filesystem access to a subdirectory, prohibit network communication, or avoid privilege escalation through other system calls.  
...
The trap-handling lwC can use the \lstinline{lwcSyscall} syscall execute any system call \textbf{in the context of the trapping lwC}:
...
We restrict ourselves to an example of a fictional application called \newcommand\appname{\textit{convert.io}\xspace} \appname which will use lwC-based \textit{application compartmentalization} to increase confidentiality of user data and the server's TLS private key. 
...
The authors also provide a PHP extension that makes the lwC API accessible from scripts, which is required for the evaluation.~\cite{lwclibsrepo}
...
Notably, this performance gain also comes with the security benefit of handling each request in a separate address space.\cite{zend}
...
It is conceivable that the additional CPU time required by lwcSwitch causes the steeper drop in throughput for lwC-nginx.\todo{explain?}
...
\end{displayquote}
Within an application, this means that different subsystems might need to operate at different privilege and access rights to memory or global system state.
...
Access to any kind of global namespace (filesystem, inet- and unix sockets, \dots) is prohibited, which gives file descriptors the role of capabilities.
